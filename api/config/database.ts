...
const DB_FILE = process.env.VERCEL === '1' || process.env.AWS_LAMBDA_FUNCTION_NAME || process.env.SERVERLESS
  ? '/tmp/avon_database.json'
  : path.join(process.cwd(), 'avon_database.json');
  ...
  try {
    ...
    if (mockTables['user_profiles'].length === 0) {
      ...
      mockTables['user_profiles'].push({
        ...
        email: 'cherub@avonpclk.com',
        role: 'Workshop Manager',
        tags: '["Workshop Engineer", "Calibration Engineer"]',
...
function executeMockQuery(sql: string, params: any[] = []): any {
  ...
  console.log(`QUERY LOG: SQL="${sql.replace(/\s+/g, ' ')}", PARAMS=${JSON.stringify(params)}`);
  ...
  if (cleanSql.startsWith('update')) {
    ...
    if (whereMatch && params.length > 0) {
      ...
      if (setMatch) {
        ...
        if (rowToUpdate) {
          let paramIdx = 0;
          const colRegex = /(?:^|,|\s)\s*\`?(\w+)\`?\s*=\s*(\?|'[^']*'|"[^"]*"|\d+)/gi;
          let match;
          while ((match = colRegex.exec(setStr)) !== null) {
            const colName = match[1];
            const valueToken = match[2];
            if (valueToken === '?') {
              rowToUpdate[colName] = params[paramIdx++];
            } else if (valueToken.startsWith("'") && valueToken.endsWith("'")) {
              rowToUpdate[colName] = valueToken.slice(1, -1);
            } else if (valueToken.startsWith('"') && valueToken.endsWith('"')) {
              rowToUpdate[colName] = valueToken.slice(1, -1);
            } else {
              rowToUpdate[colName] = Number(valueToken);
            }
          }
          saveMockTables();
        }
  ...
  if (cleanSql.startsWith('delete')) {
    ...
    if (whereMatch && params.length > 0) {
      ...
      mockTables[tableName] = tableData.filter((row: any) => String(row[keyCol]).toLowerCase() !== String(keyVal).toLowerCase());
      if (mockTables[tableName].length !== initialLength) {
        saveMockTables();
      }
    }
    return { affectedRows: 1 };
...
class UnifiedDatabasePool implements DatabasePool {
  ...
  private async runInitialize(): Promise<void> {
    ...
    if (!config.db.host || !config.db.user) {
      logger.warn('Database connection parameters are invalid or missing. Falling back to in-memory database mock.');
    ...
    const retries = 5;
    let delay = 100;
    let isLocalConnectionRefused = false;
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        isLocalConnectionRefused = (error as any).code === 'ECONNREFUSED' && 
          (config.db.host === 'localhost' || config.db.host === '127.0.0.1');

        if (isLocalConnectionRefused) {
          break; // Avoid repeated retries and warning logs
        }
    ...
    if (isLocalConnectionRefused) {
      logger.info(`Database: Local database server not found on ${config.db.host}:${config.db.port}. Falling back to in-memory database mock.`);
    } else {
