...
import { INITIAL_INSTRUMENTS, INITIAL_CALIBRATIONS, INITIAL_TERRITORIES, INITIAL_TICKETS, INITIAL_INSTALLATIONS } from './data/initialState';
...
export default function App() {
  ...
  const [dbStatus, setDbStatus] = useState<'checking' | 'healthy' | 'unhealthy' | 'offline'>('checking');
  ...
  useEffect(() => {
    ...
    const checkHealth = async () => {
      try {
        ...
        if (!res.ok) {
          if (active) setDbStatus('unhealthy');
          return;
        }
        const data = await res.json();
        if (active) {
          setDbStatus(data.database === 'healthy' ? 'healthy' : 'unhealthy');
        }
      } catch (err) {
  ...
  const [territories, setTerritories] = useState<Territory[]>(() => {
    const saved = localStorage.getItem('avon_local_territories');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) {}
    }
  ...
  const [tickets, setTickets] = useState<ServiceTicket[]>(() => {
    const saved = localStorage.getItem('avon_local_tickets');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) {}
    }
  ...
  // Helper to generate deterministic UUID from arbitrary string
  const toUUID = (str: string, prefix: string = '00000000'): string => {
    ...
    return `${prefix}-0000-4000-8000-${hex.padStart(12, '0')}`.substring(0, 36);
  };

  // Map customers from Supabase
  const mapCustomerFromSupabase = (item: any): CustomerProfile => ({
    id: item.id,
    name: item.customer_name || item.name || '',
    departments: item.departments || ['General Laboratory', 'Hematology Department'],
    endUsers: item.endUsers || [],
    installedEquipmentCount: item.installedEquipmentCount || 0,
    totalRevenue: item.totalRevenue || 0,
    activeContracts: item.activeContracts || 0,
    feedbackNpsAverage: item.feedbackNpsAverage || 0
  });

  // Map instruments from Supabase
  const mapInstrumentFromSupabase = (item: any): Instrument => ({
    id: item.id,
    serialNumber: item.serial_no || item.serialNumber || '',
    name: item.instrument_name || item.name || '',
    model: item.model || 'Sysmex Model',
    brand: item.brand || 'Sysmex',
    department: item.department || 'Gen Lab',
    customerId: item.customer_id || '',
    customerName: item.customerName || (item.customers ? item.customers.customer_name : 'Colombo General Hospital'),
    installationDate: item.installation_date || '',
    installationStatus: item.installationStatus || 'Active',
    warrantyStart: item.warranty_start || '',
    warrantyExpiry: item.warranty_end || item.warrantyExpiry || '',
    status: item.lifecycle_status || item.status || 'OPERATIONAL',
    assignedTerritory: item.assignedTerritory || '',
    calibrationRequired: item.calibrationRequired || false,
  ...
  // Map jobs from Supabase
  const mapJobFromSupabase = (item: any): JobRecord => ({
    id: item.job_no || item.id,
    jobType: item.job_type === 'BREAKDOWN' ? 'Warranty Repair' : (item.job_type === 'PM' ? 'Warranty Service' : 'Warranty Repair'),
    status: item.status || 'Pending Assignment',
    priority: item.priority || 'Routine',
    customerName: item.customerName || (item.customers ? item.customers.customer_name : 'Colombo General Hospital'),
    brand: item.instruments?.brand || item.brand || 'Sysmex',
    model: item.instruments?.model || item.model || 'Sysmex Model',
    serialNumber: item.serialNumber || (item.instruments ? item.instruments.serial_no : ''),
    createdById: item.created_by_id || 'usr-admin',
    createdByRole: item.created_by_role || 'COORDINATING_OFFICER',
    ...
    timeline: item.timeline || [
      {
        ...
        remarks: item.engineer_notes || 'Ticket registered in system.'
      }
    ]
  });

  // Centralized save instruments state helper
  const saveInstrumentsState = (updatedInstruments: Instrument[]) => {
    setInstruments(updatedInstruments);
    localStorage.setItem('avon_local_instruments_v2', JSON.stringify(updatedInstruments));
    
    // Sync to Supabase
    if (supabase && isSupabaseConfigured) {
      Promise.all(updatedInstruments.map(async (inst) => {
        try {
          await supabase.from('instruments').upsert({
            id: toUUID(inst.id, '22222222'),
            serial_no: inst.serialNumber,
            instrument_name: inst.name,
            model_id: toUUID(inst.model, '0000aaaa'),
            customer_id: toUUID(inst.customerName, '11111111'),
            warranty_start: inst.warrantyStart || new Date().toISOString().split('T')[0],
            warranty_expiry: inst.warrantyExpiry || new Date().toISOString().split('T')[0],
            lifecycle_status: inst.status === 'Faulty' ? 'UNDER_REPAIR' : 'ACTIVE',
            company_id: 'd2ef44a1-7f5d-11f1-aa6c-3cecef704050',
            territory_id: '063e53a6-7f75-11f1-aa6c-3cecef704050',
            asset_no: inst.id
          });
        } catch (e) {
          console.error('Failed to sync instrument to Supabase:', e);
        }
      })).catch(err => console.error(err));
    }
  };

  // Fetch live queries from Supabase with graceful fallback
  const fetchLiveQueries = async () => {
    ...
    try {
      ...
      if (!custErr && dbCustomers && dbCustomers.length > 0) {
        const mappedCustomers = dbCustomers.map(mapCustomerFromSupabase);
        setCustomers(mappedCustomers);
        localStorage.setItem('avon_customers', JSON.stringify(mappedCustomers));
      }

      // 2. Fetch instruments
      const { data: dbInstruments, error: instErr } = await supabase
        .from('instruments')
        .select('*, customers(customer_name)');
      
      if (!instErr && dbInstruments && dbInstruments.length > 0) {
        const mappedInstruments = dbInstruments.map(mapInstrumentFromSupabase);
        setInstruments(mappedInstruments);
        localStorage.setItem('avon_local_instruments_v2', JSON.stringify(mappedInstruments));
      }

      // 3. Fetch service jobs
      const { data: dbJobs, error: jobErr } = await supabase
        .from('service_jobs')
        .select('*, customers(customer_name), instruments(instrument_name, serial_no)');
      
      if (!jobErr && dbJobs && dbJobs.length > 0) {
        const mappedJobs = dbJobs.map(mapJobFromSupabase);
        setJobs(mappedJobs);
        localStorage.setItem('avon_jobs', JSON.stringify(mappedJobs));
      }
    } catch (e) {
      console.error('Error in fetchLiveQueries:', e);
    }
  };
  ...
  useEffect(() => {
    ...
    if (data.jobs) {
      ...
      // Fetch live queries asynchronously right after local storage is loaded
      fetchLiveQueries();
    }
  }, []);
  ...
  const saveStateToStorage = (
    ...
    if (updatedJobs) {
      ...
      if (supabase && isSupabaseConfigured) {
        Promise.all(updatedJobs.map(async (job) => {
          try {
            await supabase.from('service_jobs').upsert({
              id: toUUID(job.id, '33333333'),
              job_no: job.id,
              job_type: job.jobType === 'Warranty Repair' ? 'BREAKDOWN' : (job.jobType === 'Warranty Service' ? 'PM' : 'OTHER'),
              priority: job.priority ? job.priority.toUpperCase() : 'MEDIUM',
              status: job.status || 'PENDING_ASSIGNMENT',
              engineer_notes: job.timeline[0]?.remarks || '',
              customer_id: toUUID(job.customerName, '11111111'),
              instrument_id: toUUID(job.serialNumber, '22222222'),
              company_id: 'd2ef44a1-7f5d-11f1-aa6c-3cecef704050',
              territory_id: '063e53a6-7f75-11f1-aa6c-3cecef704050'
            });
          } catch (e) {
            console.error('Failed to sync job to Supabase:', e);
          }
        })).catch(err => console.error(err));
      }
    }
    ...
    if (updatedCustomers) {
      ...
      if (supabase && isSupabaseConfigured) {
        Promise.all(updatedCustomers.map(async (cust) => {
          try {
            await supabase.from('customers').upsert({
              id: toUUID(cust.id, '11111111'),
  ...
  const handleAddTicket = (newTicket: ServiceTicket) => {
    ...
    setTickets(updated);
    localStorage.setItem('avon_local_tickets', JSON.stringify(updated));
    handleLogAudit('CREATE_TICKET', undefined, undefined, `Created service ticket ${newTicket.ticketNumber} for instrument S/N ${newTicket.serialNumber}`);
  };

  const handleUpdateTicketStatus = (id: string, status: any) => {
    const updated = tickets.map(t => t.id === id ? { ...t, status } : t);
    setTickets(updated);
    localStorage.setItem('avon_local_tickets', JSON.stringify(updated));
    handleLogAudit('UPDATE_TICKET_STATUS', undefined, undefined, `Updated service ticket ${id} status to ${status}`);
  };

  const handleUpdateTicketPrice = (id: string, cost: number) => {
    const updated = tickets.map(t => t.id === id ? { ...t, estimatedCost: cost } : t);
    setTickets(updated);
    localStorage.setItem('avon_local_tickets', JSON.stringify(updated));
    handleLogAudit('UPDATE_TICKET_PRICE', undefined, undefined, `Updated estimated cost for ticket ${id} to LKR ${cost}`);
  };

  const handleApproveTicketBilling = (id: string) => {
    const updated = tickets.map(t => t.id === id ? { ...t, isBilled: true, status: 'CLOSED' as any } : t);
    setTickets(updated);
    localStorage.setItem('avon_local_tickets', JSON.stringify(updated));
    handleLogAudit('APPROVE_TICKET_BILLING', undefined, undefined, `Approved billing and closed service ticket ${id}`);
  };

  const handleAssignTicketEngineer = (id: string, engineerId: string, engineerName: string) => {
    const updated = tickets.map(t => t.id === id ? { ...t, assignedEngineerId: engineerId, assignedEngineerName: engineerName, status: 'DIAGNOSING' as any } : t);
    setTickets(updated);
    localStorage.setItem('avon_local_tickets', JSON.stringify(updated));
    handleLogAudit('ASSIGN_TICKET_ENGINEER', undefined, undefined, `Assigned engineer ${engineerName} to service ticket ${id}`);
  };

  // Territory handlers
  const handleAddTerritory = (newTerr: Territory) => {
    const updated = [newTerr, ...territories];
    setTerritories(updated);
    localStorage.setItem('avon_local_territories', JSON.stringify(updated));
    handleLogAudit('CREATE_TERRITORY', undefined, undefined, `Created territory ${newTerr.name} (${newTerr.territory_code})`);
  };

  const handleUpdateTerritory = (updatedTerr: Territory) => {
    const updated = territories.map(t => t.id === updatedTerr.id ? updatedTerr : t);
    setTerritories(updated);
    localStorage.setItem('avon_local_territories', JSON.stringify(updated));
    handleLogAudit('UPDATE_TERRITORY', undefined, undefined, `Updated territory ${updatedTerr.name} (${updatedTerr.territory_code})`);
  };
  ...
  const handleDeleteTerritory = (id: string) => {
    ...
    setTerritories(updated);
    localStorage.setItem('avon_local_territories', JSON.stringify(updated));
    handleLogAudit('DELETE_TERRITORY', undefined, undefined, `Deleted territory ID: ${id}`);
  };
  ...
  const handleUpdateInstallation = (updated: Installation) => {
    ...
    if (updated.status === 'Completed' && updated.warrantyCardUpdated && updated.warrantyStart && updated.warrantyExpiry) {
      ...
      if (instIndex !== -1) {
        ...
        saveInstrumentsState(updatedInstruments);
      } else {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
        ...
        return (
          <CalibrationManagement
            ...
            onUpdateInstruments={(updatedInsts) => {
              saveInstrumentsState(updatedInsts);
            }}
          />
        );
      case 'amc-management':
      ...
      case 'v4_design_system':
        return <EnterpriseDesignSystemV4 />;
      case 'v3_architecture':
        return <RBACArchitectureV3 />;
      case 'architect':
        return <ArchitectHub />;
      case 'service':
        return (
          <ServiceRepairManagement
            ...
            onAssignEngineer={handleAssignTicketEngineer}
            currentUser={activeUser}
          />
        );
      case 'territory':
      ...
      <header className="bg-slate-900 text-white shrink-0 shadow-md border-b border-slate-800 sticky top-0 z-40">
        <div className="px-6 py-3.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            {dbStatus === 'healthy' && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-950/40 border border-emerald-800/60 rounded-xl text-emerald-400" title="Connected to PostgreSQL Database">
                <Database className="w-3.5 h-3.5 text-emerald-400 animate-pulse" />
                <span className="text-[10px] font-mono font-extrabold tracking-wider uppercase">DB: ONLINE</span>
              </div>
            )}
            {dbStatus === 'checking' && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-950/40 border border-amber-800/60 rounded-xl text-amber-400" title="Checking database connection status...">
                <Database className="w-3.5 h-3.5 text-amber-500 animate-spin" />
                <span className="text-[10px] font-mono font-extrabold tracking-wider uppercase">DB: CHECKING</span>
              </div>
            )}
            {(dbStatus === 'unhealthy' || dbStatus === 'offline') && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-rose-950/40 border border-rose-800/60 rounded-xl text-rose-400" title="Database offline. Using mock database fallback.">
                <Database className="w-3.5 h-3.5 text-rose-500" />
                <span className="text-[10px] font-mono font-extrabold tracking-wider uppercase">DB: OFFLINE</span>
              </div>
            )}
