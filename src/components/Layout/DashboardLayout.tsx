      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative bg-mesh-colorful">
        ...
        <header className="h-16 bg-white/80 backdrop-blur-md border-b border-indigo-100 flex items-center justify-between px-4 sm:px-6 shrink-0 relative z-30 shadow-sm">
          ...
          <div className="flex items-center gap-2 sm:gap-4 shrink-0">
            ...
            {dbStatus === 'healthy' && (
              <div className="hidden sm:flex items-center gap-1.5 px-3 py-1 bg-emerald-50 border border-emerald-200 rounded-full text-emerald-800" title="Connected to PostgreSQL Database">
                <Database className="w-3.5 h-3.5 text-emerald-600 animate-pulse" />
                <span className="text-[10px] font-mono font-bold tracking-wider uppercase">DB: ONLINE</span>
              </div>
            )}
            {dbStatus === 'checking' && (
              <div className="hidden sm:flex items-center gap-1.5 px-3 py-1 bg-amber-50 border border-amber-200 rounded-full text-amber-800" title="Checking database connection status...">
                <Database className="w-3.5 h-3.5 text-amber-600 animate-spin" />
                <span className="text-[10px] font-mono font-bold tracking-wider uppercase">DB: CHECKING</span>
              </div>
            )}
            {(dbStatus === 'unhealthy' || dbStatus === 'offline') && (
              <div className="hidden sm:flex items-center gap-1.5 px-3 py-1 bg-rose-50 border border-rose-200 rounded-full text-rose-700" title="Database offline. Using mock database fallback.">
                <Database className="w-3.5 h-3.5 text-rose-600" />
                <span className="text-[10px] font-mono font-bold tracking-wider uppercase">DB: OFFLINE</span>
              </div>
            )}
