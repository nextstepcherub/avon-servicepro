...
it('should perform the complete Service Job workflow, assign engineers, log reports, record measurements, and evaluate KPIs', async () => {
  ...
  console.log('DEBUG: Bob performance:', JSON.stringify(performance, null, 2));
  const { data: bobsJobs } = await jobRepository.findAll({ filters: { assignedEngineerId: engineerId } });
  console.log('DEBUG: Bob jobs:', JSON.stringify(bobsJobs, null, 2));
