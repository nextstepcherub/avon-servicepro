...
export default function InstrumentRegistry({ 
  ...
  const [currentTab, setCurrentTab] = useState<'registry' | 'supabase' | 'nextjs'>('nextjs');
  ...
  const [activeProfileTab, setActiveProfileTab] = useState<'overview' | 'lifecycle' | 'history' | 'interactive'>('overview');
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.serialNumber || !formData.name || !formData.model || !formData.customerId) {
      ...
      {profileInstrument && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-xs flex items-center justify-end z-50 animate-fade-in pr-0">
          <div className="bg-white border-l border-slate-200 h-full w-full max-w-2xl shadow-2xl flex flex-col justify-between animate-slide-in p-0">
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-6 text-xs font-sans text-slate-600">
