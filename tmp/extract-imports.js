...
function scanFile(filePath) {
  ...
  const importRegex = /import\s+\{[\s\S]*?\}\s+from\s+'\.\.\/(controllers|middleware|validators|services|repositories)\/([^']+)'/g;
  let match;
  while ((match = importRegex.exec(content)) !== null) {
    const type = match[1]; // e.g. controllers
    const file = match[2]; // e.g. auth.controller
    const importedText = match[0];
    
    // Extract individual imports inside { ... }
    const braceMatch = importedText.match(/\{([\s\S]*?)\}/);
    if (braceMatch) {
